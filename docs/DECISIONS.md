...
| 20 | M4 | B | **AI-produced narrated video lessons (prototype)**, brought forward from #13 by the human: grounded LLM storyboard (numbers must exist in manual/job facts) → edge-tts voice (gTTS fallback) → Pillow slides + libass-shaped Indic text → ffmpeg MP4. Not generative text-to-video (would invent machinery/unsafe actions). Bundled Noto fonts (SIL OFL). Real product renders in the cloud | Selling point: per-operator lessons in their language from the manual + today's job |
